...
namespace AdnRme
{
    ...
    internal class CmdElectricalSystemBrowser : IExternalCommand
    {
        ...
        /// </summary>
        private class NumericalComparer : IComparer<string>
        {
        ...
        /// </summary>
        private class PanelCircuitComparer : IComparer<string>
        {
            public int Compare(string x, string y)
            {
                int d = x.CompareTo(y);
        ...
        public class PanelSystemComparer : IComparer<string>
        {
            ...
            private int CompareCommaDelimitedIntegerLists(string x, string y)
            {
                int nx = x.IndexOf(',');
                int ny = y.IndexOf(',');
          ...
          IDictionary<string, List<Element>> mapPanelAndSystemToEquipment)
        {
            ...
            {
                Debug.Assert(1 == systems.Size/*Count*/, "expected equipment to belong to one single panel and system");
    ...
    [Transaction(TransactionMode.ReadOnly)]
    internal class CmdElectricalHierarchy : IExternalCommand
    {
        public Result Execute(
          ExternalCommandData commandData,
          ref string message,
