      ...
      Element parent )
    {
      ...
      foreach( Connector c in connectors )
      {
        ...
        public Result Execute( ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            Debug.WriteLine($"Retrieved {n} electrical equipment instance{Util.PluralSuffix(n)}{Util.DotOrColon(n)}");
            //
            // determine which equipment has parents;
            // the remaining ones are root nodes:
            //
            Dictionary<ElementId, ElementId> equipmentParents = new Dictionary<ElementId, ElementId>();
            ...
            foreach (FamilyInstance fi in equipment)
            {
                foreach (Connector c in fi.MEPModel.ConnectorManager.Connectors)
                {
                    ...
                    {
                        Debug.Assert(null != c2.Owner, "expected valid connector owner");
                        Debug.Assert(c2.Owner is ElectricalSystem, "expected panel element to be electrical system");

                        ElectricalSystem eq = c2.Owner as ElectricalSystem;
                        foreach (Element e2 in eq.Elements)
                        {
                            Debug.Assert(e2 is FamilyInstance, "expected electrical system element to be family instance");
                            if (e2.Id.Equals(fi.Id)) { continue; }
                            if (equipment.Exists(
                              delegate (Element e)
